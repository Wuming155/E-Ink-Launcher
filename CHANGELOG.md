...
- 新增「文字加粗」设置：开启后桌面应用名称以及底部状态栏文字（时间、页码、电量百分比等）统一加粗显示，显著改善弱光反光环境与低分辨率墨水屏下的笔画可读性
- 优化电池电量显示：采用墨水屏经典的纯黑白横向电池图标，并增加 14sp 大字号电量百分比数字，解决原微型圆环内部小字体模糊不易读的问题
...
- 新增单元测试（首批）：覆盖自定义顺序与自定义名称序列化的编解码、JSON 特殊字符往返及旧格式迁移

### 变更

- 自定义应用名称的存储由「`\u0001`+换行」拼接改为 JSON 格式，名称可含换行、控制字符等任意字符，旧格式数据读取时自动迁移；自定义顺序仍为换行分隔的包名列表（包名不含换行，格式安全）
- 抽取 `ConfigCodec` 统一承载持久化序列化逻辑（纯静态、无 Android 依赖）
...
- 修复 Android 10+ (API ≥ 29) 无法直接开关 WiFi 导致点击无响应的问题（适配 `Settings.Panel.ACTION_WIFI` 快捷面板），并过滤未授权定位时显示的 `<unknown ssid>` 占位符
- 修复同一应用存在多个桌面入口（同包名多个 Activity）时，布局锁定/布局调整后重复图标的相对位置随系统查询顺序漂移的问题：顺序快照与交换均取首次出现，排序索引不再被后项覆盖
